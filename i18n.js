...
define([], function () {
    ...
    var STRINGS = {
        fr: {
            ...
            "cfg.diskgate.tag.flag": "Suggérer à supprimer les enregistrements visionnés quand le seuil disque est franchi (genre « AI Delete »)",
            "cfg.diskgate.tag.desc": "Option opt-in (décochée par défaut). Quand l'espace libre passe sous le seuil, les enregistrements déjà visionnés sont tagués du genre « AI Delete », du plus ancien au plus récent, jusqu'à ce que leur suppression ramène le disque au-dessus du seuil (avec marge). Pure suggestion — le plugin ne supprime jamais rien : filtrez la bibliothèque des enregistrements par ce genre, multi-sélectionnez puis supprimez. Chaque passe retire d'abord les tags précédents (si vous avez libéré de l'espace, tout disparaît). Les enregistrements non visionnés ne sont jamais tagués.",
        ...
        en: {
            ...
            "cfg.tonight.playlist.desc": "Opt-in, independent of the genre and collection above (all three can coexist). Maintains an \"AI Tonight\" Emby playlist filled with the recommended items (unwatched recordings + library) — continuous playback straight from any Emby client. Reset on each run: the playlist is emptied then filled with the current recos (it exactly reflects today's recommendations). Public (visible to the household), owned by the user below. The scheduled task (3 AM) also empties the playlist (entries removed, shell kept).",
            ...
            "cfg.diskgate.tag.flag": "Suggest watched recordings for deletion when the disk threshold is crossed (\"AI Delete\" genre)",
            "cfg.diskgate.tag.desc": "Opt-in (off by default). When free space drops below the threshold, watched recordings are tagged with the \"AI Delete\" genre, oldest first, until deleting them brings the disk back above the threshold (with margin). Pure suggestion — the plugin never deletes anything: filter the recordings library by this genre, multi-select and delete. Each pass first removes previous tags (if you freed space, everything clears). Unwatched recordings are never tagged.",
