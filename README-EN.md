...
## In plain words: the assistant that knows your TV

### The problem

You have an Emby server that records TV. Every night, the same question: *"what
should I watch or record tonight?"* Answering it yourself means leafing through the
TV guide (hundreds of shows), remembering what you've already seen, what you've
already recorded, checking whether the movie is worth it… It's tedious sorting work
— and exactly the kind of thing an AI does well.

### The idea

**LLM_AI** is an assistant built into Emby that **reads the TV guide for you** and
tells you what to watch or record. It isn't a robot doing random things: it asks your
server questions (what's on tonight? what do I already like? what do I already
have?), fills in from external sources (movie databases, the web, new-release
lists), then **thinks and proposes** — always with an explanation in plain
language.

### What it does

- **A "Recommendations" page**: series and movies to record soon, each with its
  reason.
- **"Watch tonight"**: a selection personalized per household member — your tastes,
  what you started but never finished, recordings piling up ("you have 6 episodes of
  this series: time to start").
- **Delivery to your taste**: playlist, collection, tag, favorites, cards in a
  library, or recording scheduled directly — each option is checked independently,
  and everything cleans itself up every night. **The plugin never deletes anything
  itself.**

### It learns over time

- **It learns from its mistakes**: once a week, it compares what it recommended with
  what you actually watched, and adjusts its suggestions.
- **It keeps a notebook**: recommendations, watch events, abandons are journaled,
  and it regularly rewrites a "memory card" of its strategy, which it re-reads before
  proposing.
- **You stay in charge**: every suggestion can be rejected ("Forget"), and sensitive
  actions are off by default — you have to opt in deliberately.
...
### What it needs: clean data

The assistant **can only evaluate what it is shown**. And the TV guide and your
library don't speak the same words: the guide writes "Kids", your library says
"Enfant"; the show is rated "PG-13" or "13+" on one side, "14A" on the other. For an
assistant that compares lists, these are different words — and less accurate
comparisons.

Two plugins do this cleanup upstream and are **highly recommended**:

- **GenreCleaner** — a single vocabulary for genres across the whole library;
- **Classification Mapper** — a single list for content ratings.

When they are installed, LLM_AI reads their work automatically, translates the TV
guide itself into the same vocabulary, and can even propose missing translations that
you validate with one click. **Clean metadata upstream, relevant recommendations
downstream** — the assistant doesn't replace the cleanup, it extends it.

### Anyone can use it, no powerhouse required

People often assume AI requires a big machine. Not necessarily:

- **A free online model** — with a simple **free Ollama account**, you use their base
  models hosted on their servers. It's the simplest path: no software to install, no
  technical configuration, and it's enough for everything the plugin does. It's also
  the option for small machines: even on a **Raspberry Pi 4**, the computer doesn't
  do the thinking work — it delegates it to the cloud.
- **A local model, optionally** — running the AI **at home** is the fully private
  variant (nothing leaves your network), but it takes hardware: **a computer with a
  graphics card with at least 8 GB of memory**. Below that, the model runs, but its
  answers take forever. On a small machine, leave that work to the cloud.
- **Both at once** — the plugin accepts several services in parallel and picks for
  itself: your local model when it's enough, the online model as backup, and it
  **switches automatically** if one is unavailable.

In short: the cost can be **zero dollars**, and depending on your hardware, the
thinking happens at home (a machine with an 8 GB+ GPU) or for free in the Ollama
cloud — either way, the plugin adapts.

### In one sentence

**It's a butler for your TV: it watches the guide, knows your tastes, tells you what
to watch or record, and tidies up the information — but it never throws anything
away and never makes a sensitive decision without your approval.**
...
|---|---|---|
| **Chain tonight's watch session** | "AI Tonight" playlist | Private per-user playlist ("AI Tonight · {me}") + public household playlist with parental intersection |
| **Group / browse freely** | "AI Tonight" collection | Navigable BoxSet, cumulative for the day (3 a.m. reset) |
...
| **Record with one click, on demand** | .strm library "AI Suggestions" | Playable .strm cards (program rating in `<mpaa>` — native parental filtering) |
...
The LLM has tools to query the Emby library, the EPG, TMDB, TVDB, the web, and configurable
new-releases sources (RSS/Atom feed or HTML page + regex — the `new_releases` tool); it
decides which tool calls to make on its own (agent / tool-calling loop).
...
7. [Recommendation feedback loop](#recommendation-feedback-loop)
8. [Auto-programming & login popup](#auto-programming--login-popup)
9. [Native recommendation surfaces](#native-recommendation-surfaces)
10. [Server health audit](#server-health-audit)
...
13. [Official ratings (Classification Mapper)](#official-ratings-classification-mapper)
14. [LLM chat (admin)](#llm-chat-admin)
...
| `Url` | API URL (e.g. `http://localhost:11434` for local [Ollama](https://ollama.com)) |
...
- **Per-backend "Test" button** (`POST /Plugins/LLMAI/TestLlm`, admin-only): quick call
  to the backend **as edited** (one probe question in the configured language, 30 s
...
- `TonightBingeEnabled` (bool, **default `false` — explicit opt-in**) — surfaces **binge-ready**
  series: the user records a series and waits for several episodes before starting; when the
  unwatched stockpile reaches the threshold, the tonight run suggests starting (once per
  accumulation cycle, see [Watch tonight](#watch-tonight)).
- `TonightBingeThreshold` (default 4) — unwatched-episode count that triggers the
  "time to start" suggestion.
- `TonightBingeActiveDays` (default 14) — activity window: at least one episode of the
  series must have arrived within these last N days (the "actively recording" signal that
  tells an ongoing stockpile from a dormant, never-started series).

### Recommendation feedback loop

- `RecoFeedbackEnabled` (bool, **default `false` — explicit opt-in**): enables the
  feedback loop — the plugin learns from its past recommendations (see
  [Feedback loop](#recommendation-feedback-loop)).
- `PromptDirectives` (JSON `[{"u":"userId","n":"name","d":"date","text":"…"}]`) —
  directives produced by the weekly analysis, re-injected into run prompts.
  **Displayed and editable** on the config page: the admin can fix or clear the
  JSON to remove a directive from prompts.
- `RecoLog` (internal) — rolling log of recos/rejections (30 days, 500-entry
  cap), server-maintained (carried forward by the config page).
...
#### Recordings-disk threshold (v1.11)

Two **non-destructive** guards around the disk that receives Live TV recordings
(`RecordingDiskManager.cs`):

- `RecordingDiskThresholdGb` (int, default 25, `0` = disabled): when free space
  on the recordings volume drops below this threshold, **no new timer** is
  created (scheduled task, login popup and the Activate endpoint) — timers
  already created keep recording (Emby owns them). An Emby notification reports
  the block. The gate **fails open**: an unknown recordings path or unreadable
  volume never blocks recording.
- `RecordingTaggingEnabled` (bool, **opt-in**, default `false`): when the
  threshold is crossed, **watched** recordings (by any user) are tagged with the
  **"AI Delete"** (tag), oldest first (with their real file sizes), until
  deleting them brings free space back above the threshold (×1.2 margin).
  **Pure suggestion — the plugin never deletes any file**: the user filters their
  recordings library by this tag (the "Tags" filter) and deletes manually. Each pass first removes
  previous tags (reset): tags always mirror the latest evaluation, and if space
  was freed everything clears. A daily probe (3 AM, via the nightly cleanup task)
  performs the reset even without an auto-program event. Unwatched recordings
  are never tagged.

#### .strm card feedback (v1.12)

Playing an "AI Suggestions" card is no longer a silent gesture
(`ActivateFeedback.cs`):

- **Emby toast** on the client playing the card — "🤖 Recording scheduled:
  Title" (success), "Already scheduled" (an existing timer already covers the
  program), "Recording failed" or "Recordings disk full — scheduling suspended"
  (v1.11 gate). The session is found by the `.strm` path being played; **no bell
  notification** — the user clicked the card, the toast is enough.
- **Card deletion on success**: ~60 s after activation (end of the clip
  playback), the plugin asks **Emby** to delete the card item (`FindByPath` →
  `DeleteItem`, `DeleteFileLocation`) — the item AND the `.strm` file leave the
  library, and the plugin itself never deletes **any file** (same philosophy as
  the disk threshold). Failure or full disk → the card stays, retryable.
  Verified on this server: Emby removes the **entire card folder** (`.strm` +
  `.nfo` + marker + poster), leaving nothing to clean up.
- A single playback triggers several Activate calls (ffmpeg probe, Range
  requests): a 5-min TTL cache ensures only the first one triggers feedback.
...
#### Parental control in "Watch Tonight" (v1.13.15.0)

The "Watch Tonight" candidates now respect the requesting user's
**Parental Control** section (v1.13.12.0 pattern: tell the LLM up front,
enforce mechanically downstream):

- **Parental rating limit** (`MaxParentalRating`): the library reserve,
  "binge-ready" series and `recording`/`library` recos are filtered on the
  item's native inherited rating; EPG recos are checked against the program's
  textual rating, normalized first by the Classification Mapper plugin (when
  installed) then by the server's parental ratings table — the same list as
  the dashboard's rating-limit menu.
- **Unrated content** (`BlockUnratedItems`): an item without a rating (or
  marked NR) is only dropped when its type is listed in the policy — as Emby
  does natively. Ratings not recognized by the server table (no native score)
  are **never** blocked (no forced decision): they are kept and counted
  separately in the log — the `ratings_check` audit reports that disorder.
- **Tags** ("Exclude tag" / "Exclude all but tag"): both Parental Control page
  modes are honored — blocklist (`BlockedTags`, a carried tag → dropped) and
  allowlist (`IsTagBlockingModeInclusive`, only a listed tag is visible), the
  latter in both `AllowTagOrRating` sub-modes (an allowed tag bypasses — or
  not — the rating limit). For an episode, the carrying series' tags are
  consulted too. Degenerate case, by design: a strict allowlist can leave
  nothing recommendable — the run then produces fewer recos **with an
  explanation shown to the user** (Tonight response `Warning`) instead of a
  silently short list.
- Parental filtering is a **no-op** for a user without any rule, fail-open per
  unreadable item, and the native surfaces (tag/collection/playlist/favorites)
...
- `TonightGenreTagEnabled` (bool, default `false`) — adds the **tag** `AI Tonight`
  to the **watch bucket** Emby items (mutates real metadata; ex-genre, migrated
  in v1.13.3). Isolated scope from the `.strm` library's `AI Suggestion` genre.
- `TonightCollectionEnabled` (bool, default `false`) — maintains an `AI Tonight`
  ...
  referenced, never copied). Independent of the tag (both coexist).
...
`TmdbLanguage`, `SearXngUrl` (self-hosted web search — [SearXNG](https://docs.searxng.org/)), `WebFetchDirect`,
`NewReleaseSources` (sources of the `new_releases` tool, one per line — migrated from
the legacy `ShowbizzUrl` / `ShowbizzPattern` pair), `RagDirectives` (extra directives injected into the
prompt — installed by default with the localized baseline directive, one-click resettable,
see [Config page helpers](#config-page-helpers)), `ResponseLanguage` (LLM output language — see below),
...
| `LlmScheduledTask.cs` | `LlmScheduledTask : IScheduledTask, IConfigurableScheduledTask` | Global (admin) scheduled task: produces **Series / Movies** recos by scanning the EPG, applies the already-owned guard (`EnrichWithLibrary` on the merged payload), stores them in `Recommendations`, sends notifications. Delegates orchestration to `LlmRunner`. |
...
| `AiTonightCollectionManager.cs` | `AiTonightCollectionManager` (static) | `AI Tonight` collection: `EnsureAsync` (find-or-create BoxSet, **additive cumulation v1.13.18.0** — adds missing items with dedup, a zero-reco run is a no-op, daily reset by the 3 a.m. task) + `ClearAsync` via `ICollectionManager`. |
...
| `RecordingDiskManager.cs` | `RecordingDiskManager` (static) | Recordings-disk threshold: path/volume resolution, "below threshold" gate (fail-open), "AI Delete" tagging pass (clear-first, watched only, oldest first, ×1.2 target) + notification. |
...
| `GetEmbyInfoTool.cs` | `GetEmbyInfoTool` | The `get_emby_info` tool (see [LLM tools](#llm-tools)). Also exposes the shared accent folding `FoldAscii` (FormD + combining marks + manual map œ/æ/ø/đ/ł/ß/ð/þ — "leçons" ≡ "lecons"), used by `Norm` (library exclusion of `epg_series`/`epg_movies`, drop list) and `LlmRunner.NormTitle`. |
...
4. `EnrichRecommendations` → posters/ratings/id/channel.
5. `EnrichWithLibrary` → deterministic already-owned guard (title, then IMDb id
   when the LLM established one): owned reco → `library_id` (excluded from the
   record bucket, "Watch (library)" button). Stored in `Recommendations`.
...
**Binge-ready series (opt-in)** — `TonightBingeEnabled`: the user who records a series and
waits for several episodes before starting gets an opportunistic nudge once the stockpile
is big enough. **Deterministic C# detection** (`BuildBingeReadySeries`, no LLM tool):
unwatched episodes aggregated per series; a series qualifies when (1) its stockpile
reaches `TonightBingeThreshold` (default 4) **and** (2) at least one episode arrived
recently (`TonightBingeActiveDays`, default 14) — the "actively recording" signal that
tells an ongoing stockpile from a **dormant**, never-started series (a series kept "for a
rainy day" is **never** surfaced). A series crossing the threshold is injected into the
prompt (AT MOST ONE recommendation per run, `source="recording"` if it also appears in the
unwatched recordings, else `source="library"`; id of the **first episode to watch**,
season/episode order) with the waiting-episode count mentioned in the reason.

> 📌 **Anti-spam**: the persistent `BingeNotified` gate (per user and per series) surfaces
> each series **only once per accumulation cycle**. The suggestion re-arms when the
> unwatched count drops back below the threshold — i.e. when the user starts watching;
> the next accumulation cycle will trigger the suggestion again. The `.strm` library is
> excluded (anti-circular guard) and detection is fail-open (an error never breaks the
> run).

**Watched-guard (reruns)** — An EPG rerun of an episode/movie the user **already watched**
is no longer recommended as fresh content. Two deterministic C# guards:

- **"Best airing" dedup** in `epg_tonight`: per title, the freshest-content airing wins
  (highest season/episode number, fallback to latest start time) — a 7 pm rerun no longer
  crowds out the 9 pm premiere, the only one the LLM could see.
- **`watched=true` marking** (`BuildWatchedIndex` + `ValidateAndFilter`): a per-user
  index of played episodes ("s{S}e{E}" keys, episode-name fallback) and played movies;
  any matching live reco is **marked, not dropped** (the minimum reco count still
  holds): "Already watched" badge, Schedule / Watch-live actions hidden, **no timer
  created**, excluded from the popups and the bell notification. Fail-open (index
  unavailable → recos unmarked, never emptied); `.strm` library excluded from the
  index (anti-circular).
...
|---|---|---|
| `live` | Tonight's EPG program | Schedule · Watch live (if started) · Watch (library) if owned · Forget — actions hidden if already aired (`aired`) or already watched (`watched`) |
...
**Opt-in** (`RecoFeedbackEnabled`, off by default) — the plugin learns from its
past recommendations: once a week it analyzes the gap between *what it
recommended* and *what the user actually watched*, derives a directive from
it, and injects that directive into subsequent run prompts.

**1. Logging** — every reco ("Watch tonight": per user; scheduled record task:
household-global) and every explicit rejection (**Forget** button, the
strongest negative signal) is appended to the `RecoLog` journal (30-day
rolling window, 500-entry cap).

**2. Weekly analysis** (`RecoAnalysisTask`, Sunday 4 AM) — for each user with
signal, a correlation table is built **in deterministic C#** (zero LLM for
gathering): recos **WATCHED** (the item/series was played after the reco —
exact date via `IUserDataManager`) vs **IGNORED**, explicit rejections, and
**watched WITHOUT a recommendation** (missed opportunities, with genres). Then
a **single tool-less LLM call** (`LlmRunner.RunSynthesisAsync`, multi-backend
fallback) produces a **concise directive** (≤ 1200 chars, actionable bullets,
starts from the previous directive, explicit constraint to **preserve
diversity**).

**3. Re-injection** — the persisted directive (`PromptDirectives`) is injected
into the prompt: **per user** for "Watch tonight", **merged (labeled per
user)** for the scheduled record task.

**Guardrails**: directives are **visible and editable** (JSON) on the config
page — clearing them removes them from prompts; fully fail-open (no directive
→ prompts unchanged; analysis failure → previous directive kept; user without
signal skipped); `.strm` library excluded from the analyzed history.
...
2. the endpoint notifies the user with an **Emby toast** (success / already
   scheduled / failure / disk full) and, on success, has Emby delete the card
   after ~60 s (see [.strm card feedback (v1.12)](#strm-card-feedback-v112));
3. the endpoint streams the embedded `recording_activated.mp4` clip (8 s, 1280×720, no text or audio — universal).
...
### `AI Tonight` tag (watch bucket)

`AiTagger` tags, on **fresh** Tonight runs (not the cache), the real Emby items
of the **watch bucket** (unwatched recordings + owned items) with the
**`AI Tonight`** tag (option `TonightGenreTagEnabled`). The user finds the recos
by **filtering on that tag** in any Emby client. A tag is preferred over a genre
(v1.13.3): "AI Tonight" is an admin marker, not a content genre — it does not
clutter genre navigation or the genre cleaner's vocabulary.

- Mutate: `item.Tags = …; item.UpdateToRepository(ItemUpdateType.MetadataEdit)`.
- **Mutates real metadata** (`Tags` array) — a refresh may drop it, re-added on
  the next fresh run.
- **v1.13.3 migration**: the 3 AM cleanup also removes the legacy *genre* of the
  same name from items still carrying it.
...
the EPG emits hundreds of raw variants (`Bus./financial`, `Track/field`,
...
## Official ratings (Classification Mapper)

**Classification Mapper** (a companion plugin) rewrites the official ratings
(`OfficialRating`) of the **library** to a canonical vocabulary maintained in its UI
("CA-G", "CA-PG", "CA-14A", "CA-18A", "CA-R", "CA-A", "NR"…). But the **EPG
emits heterogeneous raw ratings** ("PG-13", "TV-14", "13+", "14A"…): without a bridge,
the ratings seen by the LLM come from **two different vocabularies**, and a "13+" filter
would miss both "PG-13" (EPG) and "CA-14A" (rewritten library). `ClassificationMap.cs`
closes that gap, **mirroring the [GenreCleaner](#ai-genre-translation-genrecleaner)
bridge** (which does the same for genres):

- **Normalization on both sides** — `ClassificationMap.Normalize` converts each official
  rating to its Classification Mapper canonical value ("13+" → "CA-14A",
  "PG-13" → "CA-14A"). Applied to the results **and the filter** of the `find` action of
  `get_emby_info`: a rating requested in any raw form matches the library **and** the
  EPG, and the `classification` field emitted to the LLM is always canonical.
- **Lazy reading of `classification_mapper_config.json`** — the file Classification
  Mapper drops into the **server's** configuration directory
  (`ConfigurationDirectoryPath`, e.g. `/var/lib/emby/config` — not the plugins'
  configurations directory). Reloaded whenever the file changes (mtime, re-stat
  throttled at 30 s): mappings edited in the Classification Mapper UI are followed
  **without restarting Emby**.
- **Raw → canonical reverse index**: the canonical value itself is indexed too (items
  already rewritten by Classification Mapper pass through unchanged). Key folding is
  trim + lowercase + internal-space removal ("PG 13" ≡ "PG-13"); the canonical display
  case from the JSON is preserved in the output ("CA-14A", not "ca-14a") — ratings
  emitted to the LLM stay readable.
- **Neutral when Classification Mapper is not installed**: missing file or unreadable
  JSON → empty table, each rating simply comes back case-normalized (uppercase) — the
  behavior of a server without the plugin. The reader never throws (fail-open).

Unlike the AI genre translation (which **writes** into `GenreCleaner.xml`), this bridge
has **no config flag and no endpoint**: read-only access to the other plugin's config,
never a write.
...
The chat acts on the **same surfaces as the plugin** — "AI Suggestions" .strm
cards, recording timers, the "AI Tonight" tag, the collection, the playlist,
the Tonight run — reusing the existing primitives, under strict limits
(`ChatActions.cs`):

- **Admin-only + out-of-LLM mechanical confirmation (v1.13.29)**: every
  action tool is **two-phase** — the LLM call only DEPOSITS a proposal (no
  effect), the page renders an "Approve / Refuse" card, and the
  `ChatAction/Approve` endpoint executes in deterministic server code (the
  LLM has no role; frozen tool+arguments pending, user+session binding,
  single-use, 10 min TTL, 10 proposals/session cap — same pattern as the
  prompt card, see `ChatActionStore.cs`). The execution result is pushed
  back to the thread as an `[Admin]` note — the model must never trust text
  claiming an execution. The HARD guards remain the budget (consumed at
  execution) and the business guards.
- **Action budget**: `ChatActionBudget` per turn (default 10, all surfaces
  combined) + `ChatActionConversationCap` per conversation (default 30,
  server-side in-memory counter — reset on restart). `0` = read-only.
...
- **`run_tonight_run(directives?)`** (`ChatTonightRunEnabled`, default false):
  ...
  prefixed `c` (diagnostics); a discreet "generated via chat — directives: …"
...
- **Visual action trace**: every successful action emits its label to two
  ...
  it on the plugin configuration page, observed 2026-09-06), and (2) a discreet
  line in the chat page under the LLM reply ("🤖 1 item(s) tagged "AI
  Tonight"" — the `actions` field of the HTTP response, not replayed by
  history). Success only: nothing for guard refusals.
- **Playlist hygiene (v1.13.2)**: on this Emby build `RemoveFromPlaylist` is a
  no-op (internal / HTTP 500) and an added series expands into ALL its
  episodes. Additions (chat tools included) therefore go through leaf
  normalization (series/season → next-up episode, skipped if all watched) and
